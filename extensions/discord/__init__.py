...
from utils import fish_discord, TwemojiConverter, to_image, plural, human_join
...
class Discord(Cog):
    ...
    async def emoji_group(
        ...
        embed = discord.Embed(
            timestamp=emoji.created_at, title=emoji.name, color=self.bot.embedcolor, description="**Raw text**: `<:{emoji.name}\u200b:{emoji.id}>`"
        )
