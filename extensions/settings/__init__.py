...
class Settings(Logging, Server):
    ...
    async def link_lastfm(self, ctx: Context, username: str):
        if not LASTFM_USERNAME.search(username):
            raise commands.BadArgument("Invalid last.fm username.")
...
class ManageAccountsModal(discord.ui.Modal, title="Manage Accounts"):
    lastfm = discord.ui.TextInput(
        label="Last.fm", required=False, max_length=100, placeholder="Clear to unlink"
    )
            ...
            if v:
                ...
                if col == "letterboxd":
                    ...
                    if m := LBD_URL_RE.match(v):
                        v = m.group(1)
                if col == "lastfm" and not LASTFM_USERNAME.search(v):
                    continue
                ...
                if col == "lastfm":
                    self.ctx.bot.db_cache.add_account(interaction.user.id, v)
                ...
                if col == "lastfm":
                    self.ctx.bot.db_cache.remove_account(interaction.user.id)
                removed.append(col)
        msg = []
        if added:
