...
def test_mass_permission_boundary_matches_protection_managers(monkeypatch) -> None:
    ...
    assert _can_manage_mass(_ctx(_Member(1, guild), config))
    assert _can_manage_mass(_ctx(_Member(2, guild, protection_role=99), config))
    ...
    assert not _can_manage_mass(_ctx(_Member(5, guild), config))
...
def test_mass_runtime_check_allows_protection_role_without_native_permissions(
    ...
    Mass._ensure_mass_permissions(ctx, "ban")
