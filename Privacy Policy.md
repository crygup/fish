...
Authenticated users can inspect and delete their own stored data. A full user deletion removes account links, wallet and inventory, sessions, settings, histories, reminders, game state, subscriptions, and references where the user is an actor or target. Server managers can delete guild-owned history and configuration.

Deletion hides the affected data from normal bot and website access immediately and retains a private recovery copy for 31 days. This applies to individual records, categories, and entire accounts. During that window, use `settings restore` in Fishie or **Restore all data** in the Discord page's Settings tab. Server restoration requires Manage Server permission. After the deadline, recovery is refused and the archive is purged by the hourly cleanup job. Existing downloaded files, previously sent Discord messages, and third-party copies cannot be recalled by this process.

Restoration does not reactivate revoked website login sessions or overwrite newer settings. Shared records remain hidden while another affected user's deletion request is pending. Conflicting relationships or expired one-shot tasks may remain pending rather than replacing current state. Reward limits continue to apply during the recovery window. Some security or backup records may persist temporarily where needed to prevent abuse or complete backup expiry.
